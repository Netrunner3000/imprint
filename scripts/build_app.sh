...
# ─────────────────────────────────────────────────────────────────────────────
#
# Build & install "Create & Publish.app" — a self-contained macOS application bundle
...
#   ./scripts/build_app.sh --no-install   # build only (dist/Create & Publish.app)
#
...
# in  ~/Library/Application Support/Create & Publish/  — seeded on first launch and
...
# never shows up as a second "Create & Publish" alongside the installed one.
touch "${PROJECT_ROOT}/dist/.metadata_never_index"
"$PY" -m PyInstaller --noconfirm --clean CreateAndPublish.spec
...
# keeps returning for "Create & Publish", and launching THAT copy silently runs a
...
# that guarantees exactly one "Create & Publish" app exists on disk.
rm -rf "$DIST_APP"
