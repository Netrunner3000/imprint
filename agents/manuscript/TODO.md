...
- [ ] `P1` Make one Project record the source for title, author and approved manuscript. Partial 2026-09-23: Quote Finder's **Use Project Draft** explicitly reads the selected Project's current Write text (falling back to its linked saved draft) and sets attribution from the shared byline. **Approve Write Draft…** now stores immutable numbered versions and **Use Approved Version** selects one for Quote Finder. Remaining: tie outward publishing files and retailer submissions to the chosen approved version; approval inside Imprint does not certify an external export.
