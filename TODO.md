...
- [ ] `P1` `design` `@ai` **Make Project the real object** (SUGGESTIONS #42, now PLANNED). The 2026-09-22 identity slice made the project row own work type, title, byline/brand and brief; Write restores its manuscript workspace per project. The 2026-09-23 handoff links saved drafts and EPUB/DOCX/PDF exports, gives Publishing Manager an explicit **Use Project Draft** action, and lets Audiobook convert a linked Project book while associating the finished MP3 with its starting Project. The cross-workspace item stays open: approval/version choice, Project grouping in Listen, Video and Creator asset links, and a common project view remain. Sequence and deletion rules are in `docs/projects_roadmap.md`.
