...
- [x] `P1` `feature` `@ai` **Video provider/model routes.** The Video workspace now offers current GPT Image models; Sora 2/2 Pro; Gemini Omni 1.1 and all Veo 3.1 tiers; Qwen/Wan 3.0, Prime and 2.7; Higgsfield Seedance; Pexels; and local gradient visuals. Scene models flow through vidforge; direct models create/poll/download and join the same library. Duration/aspect controls follow each API, every paid request uses a per-second reserve or provider quote, and create calls cannot silently retry into duplicate jobs. DALL·E is absent because its APIs were removed; DeepSeek, Anthropic, Kimi and Ollama are not visual providers because they expose no video-output API.
- [ ] `P0` `maintenance` `@me` **Replace the Sora 2 direct route when OpenAI names a successor.** The Sora API is deprecated and scheduled to shut down permanently on 24 September 2026. Imprint labels it as retiring and blocks submissions from that date, so this becomes a provider migration rather than a production outage. Higgsfield and the GPT Image scene pipeline remain available.
...
- [ ] `P1` `infra` `@me` **7 of the 12 repos git_autosync is configured to back up are silently skipped.** Found while chasing the push above. The *effective* config is `~/Library/Application Support/git_autosync/autosync_repos.txt` — not the copy in the git_autosync repo — and it still lists bare names from before the folder reorg: `convert_epub`, `image_tools`, `sentinel_ai`, `vpn_agent`, `unblock_tracker`, `create_and_publish`, `vidforge`. All seven now live in subdirectories (`toolbox/…`, `sentinel_fork/vpn_agent`, `imprint`, `imprint/vidforge`) or are archived, so they resolve to nothing and are logged as `SKIP (not a git repo)`. Only `backup_manager`, `git_autosync`, `sonar`, `lab_hub` and `sentinel_fork` are actually synced. **Several projects have had no automated off-machine push since the reorg**, and the nightly log reports success while doing it. Not fixed here: a peer session is working in `git_autosync`, and this is its territory.
- [x] `P2` `research` `@me` **Set the Higgsfield render rate.** Superseded: Imprint now calls Higgsfield's authenticated estimate endpoint for each exact payload, so the obsolete static `per_unit_usd.higgsfield_render` placeholder is no longer used for Creator renders.
- [x] `P0` `bug` `@ai` **Bring the Higgsfield client onto the current official API.** Fixed: key ID + secret authentication, model-specific Seedance endpoints, presigned reference-image upload, returned status/cancel URLs, `nsfw`/`canceled` terminal handling, bounded polling backoff, safe GET retries, queued cancellation, correlation IDs and local output downloads. Generation POSTs are deliberately never retried after an ambiguous timeout because the API has no idempotency key.
- [x] `P1` `feature` `@ai` **Finish the Higgsfield production loop.** Fixed: the exact provider estimate appears before approval; `creator_video_jobs` persists the request ID, endpoint, prompt version, estimate, completed charge basis, lifecycle, policy result, local path and correlation ID; media rows retain the job ID; and a render is attached to the selected Creator calendar item. Flow: persona/brief → local policy/consent → estimate → human approval → render → media library → selected calendar item → manual publish.
- [ ] `P1` `feature` `@ai` **Make Creator platform-aware instead of hardcoding OnlyFans.** Add a platform field and policy profile that records whether synthetic personas are allowed, whether the account owner must be the depicted verified creator, required AI disclosure text/tags, allowed publishing method, and the date/source of the last policy review. Keep OnlyFans drafting/manual-publish only; do not add browser automation or reverse-engineered private endpoints.
- [ ] `P1` `research` `@me` **Verify the current OnlyFans AI-content rules inside the logged-in Creator Help/Terms before launching a virtual creator.** Publicly surfaced guidance says AI-generated or altered content must clearly feature the verified creator, be visibly labelled as AI (for example `#ai`), and obey the wider content rules; that does not safely establish that a wholly fictional synthetic person can operate an account. Record the exact live wording and review date in the platform policy profile. Until verified, the compliant OnlyFans use case is a verified consenting adult using labelled AI enhancement of their own likeness; use a fully synthetic persona only on a platform whose current written rules expressly permit it.
- [ ] `P2` `feature` `@ai` **Close the Creator ROI loop.** Add per-asset campaign/channel fields and record generation cost, publish status/permalink, reach, clicks, subscriptions, PPV purchases and attributed revenue. Replace the earnings text dump with a small dashboard comparing asset, hook, channel and price-point ROI; start with manual metric entry/import before adding documented public-channel APIs.
- [ ] `P2` `research` `@me` **Verify the posting notes** in `services/social_platforms.py` against each platform's current terms before wiring credentials. They were written from general knowledge, not checked against the live APIs, and X's pricing in particular changes often.
- [ ] `P3` `feature` `@ai` Social has no per-post analytics loop — a posted item records its permalink and nothing after that. Reach and clicks would make the angle comparison real rather than a guess.
- [ ] `P2` `design` `@ai` Two rows of navigation both say "Publish": the Write workspace's stage tabs are `Draft | Publish` (author vs. manuscript agent) and the author panel's own mode tabs are `Write | Publish & Market`. Both are correct in isolation and confusing stacked. Needs a naming call, not a layout change.
...
- **Learning Centre** — seven searchable guide pages with generated screenshots, measurable income experiments, and complete workspace/agent coverage.
