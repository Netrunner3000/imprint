...
- [x] `P2` `feature` `@ai` **The image model is a control.** `services/openai_client.generate_image()` takes a model and returns PNG bytes; the Gigs panel offers `dall-e-3` (default) and `gpt-image-1`. Previously hardcoded and invisible, which is why nothing appeared to recommend DALL·E for the graphics work it was already doing. The budget guard still cannot see image spend — that half is the P1 below.
...
- [x] `P1` `research` `@ai` **Identified: it was a Claude Code session, and the gitleaks gate did cover it.** The two 2026-09-09 pushes (`fea19e5`, `db28e82`) came from a git_autosync session acting on the user's instruction to push imprint's unpushed commits; both ran `gitleaks detect` over the full history first (52 and 57 commits, no leaks) before pushing. The "every commit" premise does not survive the reflog: `git reflog show origin/main` has **5 entries total** against ~57 commits on main, with commit→push gaps of 2 days, 1 second, 31 minutes, 3 hours and 36 seconds. That is occasional manual pushing, not an automated hook — the 1-second case (`b56c80a`, 2026-08-26) is a `git commit && git push` one-liner. `.git/hooks/` is empty and the 03:00 LaunchAgent is not involved, both correctly observed. imprint stays out of `autosync_repos.txt` on purpose: autosync only ever pushes the *checked-out* branch, so it would commit WIP under a timestamp message and publish whatever feature branch happened to be current.
- [ ] `P2` `research` `@me` **Set the Higgsfield render rate** in `config/pricing.json` → `per_unit_usd.higgsfield_render`, from an actual invoice. Until then a teaser render prompts and is billed at zero.
