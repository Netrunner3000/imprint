...
- [ ] `P1` `design` `@ai` **Make Project the real object** (SUGGESTIONS #42, now PLANNED). The 2026-09-22 identity slice made the project row own work type, title, byline/brand and brief; Write restores its manuscript workspace per project. The 2026-09-23 handoffs link Write drafts/exports, give Publishing Manager **Use Project Draft**, link Audiobook MP3s and Video clips, and associate Creator content, teaser jobs and media without transferring account/consent ownership. Project Overview joins working-state counts, files and Creator content, including missing-file warnings. Audio Listen, Video Library, and Creator Calendar/Media offer explicit current-Project views while retaining All by default. Publishing Manager now stores immutable approved manuscript versions selected separately from working drafts. The P1 stays open for tying outward publishing files and retailer submissions to a chosen approved version; approval inside Imprint does not certify an external export. Sequence and deletion rules are in `docs/projects_roadmap.md`.
