...
- [ ] `P3` `docs` `@ai` Add `docs/agents/course.md` — every other agent has a reference page under `docs/agents/` opened by its panel's 📖 Docs button; the Course Generator has no panel (it's CLI-only, §5.8) so it has no Docs button either, but a reference page would still help since `run_course.py --help` is the only current documentation of its flags.
