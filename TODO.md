...
Music, Site Builder and Audiobook now own their workspace layouts and
request/result lifecycles in their own packages. Temporary control aliases
remain in the umbrella for shared integrations.
...
- [ ] `P1` `design` `@ai` **Refactor Phase 4 — panel extraction.** Foundation shipped 2026-09-14: flat implementations moved to `agents/<key>/agent.py`; package public APIs replaced private-file imports; `agents/catalog.py` now owns roster/workspace/recommendation metadata; and every visible, CLI and internal agent owns README/TODO/SUGGESTIONS files. Music, Site Builder and Audiobook now own their panels and handlers; OnlyFans was already there. Audiobook's exact-text estimate, guarded paid conversion process/result lifecycle and Stop path moved into its package on 2026-09-21, leaving only compatibility aliases for shared umbrella integrations. Remaining work is tracked per package: move the other panels and handlers out of `main.py`, then relocate services proven exclusive to one agent. See `docs/agent_project_structure.md`.
...
- [x] `P2` `design` `@ai` **Dead code sweep (all but Sora).** Done 2026-09-21: `services/model_router.py` deleted; **Auto Route actually routes now** — the button classifies the prompt through `agents.router.RouterAgent` (tested, never called before) and switches workspace instead of echoing the selection; `check_budget_before_request` deleted (float money, no callers, worse duplicate of the Decimal validator); `SubprocessWorker` deleted (imported, instantiated nowhere).
- [ ] `P2` `design` `@ai` **Delete Sora after 2026-09-24.** The API closes then; remove the catalog rows, `OPENAI_VIDEO_MODELS`, `SORA_USD_PER_SECOND` (still merged into `DIRECT_VIDEO_USD_PER_SECOND`), `sora_cost_usd`, `sora_is_retired` (no production caller), `OpenAIClientWrapper`'s Sora job methods, `OpenAIVideoWorker` (test-only), and their tests. Deliberately date-gated — not dead until the shutdown lands.
