...
- [ ] `P1` `research` `@me` **Something pushes this repo to public GitHub within ~30s of every commit, and nothing documents what.** `autosync_repos.txt` deliberately excludes `imprint` and says it is pushed by hand; `git reflog show origin/main --date=iso` shows automated pushes tracking commits since August, including two on 2026-09-09 that landed 36 and 29 seconds after their commits. The 03:00 `com.netrunner3000.git-autosync` LaunchAgent does not explain the timing and `.git/hooks/` is empty. Worth identifying, because if it is not git_autosync then the gitleaks gate is not covering these pushes. Nothing sensitive is tracked today — `.env` is ignored, only `.env.example` is committed, no keys or `.secrets` in the tree — but that is currently luck rather than a control.
