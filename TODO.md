...
- [ ] `P2` `infra` `@me` **imprint is now in autosync, and its commit history shows why that was avoided.** The fix above also added `imprint`, so 2026-09-10, -11 and -12 each produced a commit titled `autosync: <timestamp>` that swept up whatever was in the tree — including a substantial Learning Centre rewrite and two new pages, none of which has a message describing it. `AGENTS.md` still says imprint is excluded on purpose and gives the reason (autosync pushes only the checked-out branch, under a timestamp message). Either the exclusion should come back, or that paragraph should go — right now the workspace's source of truth contradicts the live config. Your call, since it is a backup-versus-history tradeoff rather than a bug.
