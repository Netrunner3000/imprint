...
- [x] `P1` `feature` `@ai` **Video provider/model routes.** The Video workspace now offers current GPT Image models, Sora 2/2 Pro, Higgsfield Seedance, Pexels and local gradient visuals. Scene models flow through vidforge; direct models create/poll/download and join the same library. Sora uses exact per-second approval and cannot silently replay a paid create request; Higgsfield obtains an exact quote first. DALL·E is absent because its APIs were removed.
