...
- [ ] `P1` `design` `@ai` **Make Project the real object** (SUGGESTIONS #42, now PLANNED). First slice landed 2026-09-22: the existing project row now owns work type, title, byline/brand and brief; Settings edits them; Write saves and restores its manuscript workspace per project and shares title/byline with that row. The cross-workspace item stays open: exports/Publishing Manager, Audiobook, Video and Creator still need durable project asset links and a common project view. Sequence and deletion rules are in `docs/projects_roadmap.md`.
