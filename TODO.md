...
- [ ] `P1` `design` `@ai` **Make Project the real object** (SUGGESTIONS #42, now PLANNED). The 2026-09-22 identity slice made the project row own work type, title, byline/brand and brief; Write restores its manuscript workspace per project. The 2026-09-23 handoffs link saved drafts and EPUB/DOCX/PDF exports, give Publishing Manager **Use Project Draft**, let Audiobook convert a linked Project book and associate its MP3, and link finished Video clips even after provider-job recovery. The cross-workspace item stays open: approval/version choice, Project grouping in the Audio/Video libraries, Creator asset links, and a common project view remain. Sequence and deletion rules are in `docs/projects_roadmap.md`.
